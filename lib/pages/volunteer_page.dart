...
import 'package:food_delivery_app/pages/volunteer_pop_screen.dart';
import 'package:provider/provider.dart';
import 'package:flutter_slidable/flutter_slidable.dart';
import 'package:food_delivery_app/components/theme_provider.dart';
import 'package:food_delivery_app/pages/add_volunteer_details.dart';
import 'package:http/http.dart' as http;
import 'package:url_launcher/url_launcher.dart';
import 'edit_volunteer_details.dart';

class VolunteerPage extends StatefulWidget {
  final String userId;
  final String location;

  const VolunteerPage({super.key, required this.userId, required this.location});
...
class _VolunteerPageState extends State<VolunteerPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> fetchVolunteers() async {
    volunteerStream = FirebaseFirestore.instance
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          Expanded(child: allVolunteerDetails()),
        ],
      ),
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => AddVolunteerDetails(
                Loc: widget.location,
    ...
    return StreamBuilder(
      ...
      builder: (context, AsyncSnapshot snapshot) {
        ...
        volunteers.sort((a, b) {
          ...
          // Check if all volunteers are available
          bool allAvailable = volunteers.every((volunteer) {
            bool isActive = volunteer["status"] == "active";
            return isActive && (volunteer["availability"]["type"] == "Full Time" || 
                   isCurrentlyAvailable(volunteer["availability"]));
          });
...
return createdAtA.compareTo(createdAtB);
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            DocumentSnapshot ds = volunteers[index];
                    ...
                    ? ActionPane(
                        ...
                        children: [
                          CustomSlidableAction(
                            ...
                            onPressed: (context) async {
                              final volunteerId = ds.id;
                              final confirmation = await showDialog(
                                context: context,
                                builder: (BuildContext context) {
                                  return AlertDialog(
                                    title: const Text("Confirm Deletion"),
                                    content: const Text("Are you sure you want to delete this volunteer entry?"),
                                    actions: [
                                      TextButton(
                              ...
                              if (confirmation == true) {
                                try {
                                  await FirebaseFirestore.instance.collection("volunteer").doc(volunteerId).delete();
                                  ScaffoldMessenger.of(context).showSnackBar(
                                        ...
                                        itemBuilder: (context, skillIndex) {
                                          return Padding(
                                            padding: EdgeInsets.only(
                                              right: skillIndex != skills.length - 1 ? 6.0 : 0,
                                            ),
                                            child: Container(
                                              decoration: BoxDecoration(
                                                color: themeProvider.isDarkMode
                                                    ? Colors.grey[700]
                                                    : lightModeColors[skillIndex % lightModeColors.length],
                                                borderRadius: BorderRadius.circular(12),
                                              ),
                                              ...
                                              child: IconButton(
                                                ...
                                                onPressed: () {
                                                  Navigator.push(
                                                    context, 
                                                    MaterialPageRoute(
                                                      builder: (context) => EditVolunteerDetails(
                                                        userId: widget.userId,
                                                        volunteerId: ds.id,
                                                        volunteerData: ds.data() as Map<String, dynamic>,
                      ...
                      onTap: () {
                        ...
                        if (isAvailable) {
                          Navigator.push(
                            context, 
                            MaterialPageRoute(
                              builder: (context) => VolunteerPopScreen(volunteerData: ds)
                            )
                          );
                        } else {
                          showDialog(
                            ...
                            builder: (BuildContext context) {
                              return AlertDialog(
                                ...
                                content: const Text("This volunteer is currently not available for service."),
                                actions: [
                                  TextButton(
